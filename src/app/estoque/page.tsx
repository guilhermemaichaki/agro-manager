...
async function fetchStockBalance(): Promise<StockBalance[]> {
  ...
  // Agrupa movimentações por produto e calcula saldo
  const balanceMap = new Map<string, { entries: number; exits: number }>();
  ...
  movements.forEach((movement) => {
    ...
    if (!balanceMap.has(productId)) {
      balanceMap.set(productId, { entries: 0, exits: 0 });
    }
  ...
  const balances: StockBalance[] = Array.from(balanceMap.entries()).map(
    ([productId, balance]) => {
      ...
      return {
        ...
        unit: product?.unit || "-",
        balance: balance.entries - balance.exits,
      };
    }
  );
...
async function fetchStockEntries(): Promise<StockMovement[]> {
  const { data, error } = await supabase
    .from("stock_movements")
    .select(`
      *,
                ...
                <Table>
                  ...
                  <TableBody>
                    {stockEntries.map((entry) => {
                      const product = entry.product as Product | undefined;
